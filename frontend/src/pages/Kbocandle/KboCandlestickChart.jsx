...
export default function KboCandlestickChart({ kboData, dark, setDark }) {
    ...
    useEffect(() => {
        ...
        if (plus) {
            ...
            main = chartType === "bar"
                ? chart.addSeries(BarSeries, { upColor: candlePalette.up, downColor: candlePalette.down, openVisible: true, thinBars: false, priceFormat })
                : chart.addSeries(CandlestickSeries, { upColor: candlePalette.up, downColor: candlePalette.down, wickUpColor: candlePalette.up, wickDownColor: candlePalette.down, borderVisible: false, priceFormat });
    ...
    useEffect(() => {
        if (!plus && !lineChart) api.current?.main.applyOptions({ upColor: candlePalette.up, downColor: candlePalette.down, ...(chartType === "candle" ? { wickUpColor: candlePalette.up, wickDownColor: candlePalette.down } : {}) });
    }, [candlePaletteId, plus, chartType, bars, calendarGaps, dark]);
...
<div><button className="candle-nav-icon" disabled={!latest} onClick={() => move(-1)} aria-label="이전 구간"><i className="bi bi-chevron-left" aria-hidden="true" /></button><button className="candle-nav-icon" disabled={!latest} onClick={() => move(1)} aria-label="다음 구간"><i className="bi bi-chevron-right" aria-hidden="true" /></button><button className="candle-nav-icon" disabled={!latest} onClick={() => zoom(1.3)} aria-label="차트 축소"><i className="bi bi-dash-lg" aria-hidden="true" /></button><button className="candle-nav-icon" disabled={!latest} onClick={() => zoom(0.75)} aria-label="차트 확대"><i className="bi bi-plus-lg" aria-hidden="true" /></button><button className="candle-nav-icon" disabled={!latest} onClick={() => api.current?.recent()} aria-label="차트 초기화" title="초기화"><i className="bi bi-arrow-counterclockwise" aria-hidden="true" /></button></div>
        ...
        <Modal show={chartSettingsOpen} onHide={() => setChartSettingsOpen(false)} centered className={`candle-settings-modal font-family-NaSqNe ${dark ? "theme-dark" : "theme-light"}`} contentClassName="candle-settings-modal-content">
            ...
            <Modal.Body>
                ...
                <label className={`candle-settings-switch-row ${timeframe !== "daily" ? "is-disabled" : ""}`}><span className="candle-settings-copy"><span>출전 경기만 표시</span><small>경기가 없거나 출전하지 않은 날은 표시하지 않습니다.</small></span><input type="checkbox" value="" className="sr-only peer candle-settings-switch-input" checked={!calendarGaps} disabled={timeframe !== "daily"} onChange={event => setCalendarGaps(!event.target.checked)} /><span className="candle-settings-switch" aria-hidden="true" /></label>
                <label className="candle-settings-switch-row"><span className="candle-settings-copy"><span>최고 · 최저 표시</span><small>차트 위에 최고점 및 최저점을 표시합니다.</small></span><input type="checkbox" className="sr-only peer candle-settings-switch-input" checked={showExtrema} onChange={event => setShowExtrema(event.target.checked)} /><span className="candle-settings-switch" aria-hidden="true" /></label>
                {!plus && <fieldset className="candle-chart-style-setting"><legend>차트 형태</legend><div className="candle-chart-style-options">
                    <button type="button" className={chartType === "candle" ? "selected" : ""} aria-pressed={chartType === "candle"} onClick={() => setChartType("candle")}><svg viewBox="0 0 34 20" aria-hidden="true"><path d="M10 1v18" strokeWidth="1.5" stroke={candlePalette.up}/><path d="M24 1v18" strokeWidth="1.5" stroke={candlePalette.down}/><rect x="6" y="5" width="8" height="10" fill={candlePalette.up}/><rect x="20" y="7" width="8" height="7" fill={candlePalette.down}/></svg>캔들</button>
                    <button type="button" className={chartType === "line" ? "selected" : ""} aria-pressed={chartType === "line"} onClick={() => setChartType("line")}><svg viewBox="0 0 16 16" aria-hidden="true" fill="currentColor"><path d="M14.817 3.113a.5.5 0 0 1 .07.704l-4.5 5.5a.5.5 0 0 1-.74.037L7.06 6.767l-3.656 5.027a.5.5 0 0 1-.808-.588l4-5.5a.5.5 0 0 1 .758-.06l2.609 2.61 4.15-5.073a.5.5 0 0 1 .704-.07"/></svg>실선</button>
                    <button type="button" className={chartType === "bar" ? "selected" : ""} aria-pressed={chartType === "bar"} onClick={() => setChartType("bar")}><svg viewBox="0 0 34 20" aria-hidden="true" fill="none"><path d="M10 1v18M5 5h5M10 13h5" stroke={candlePalette.up} strokeWidth="2"/><path d="M24 1v18M19 7h5M24 15h5" stroke={candlePalette.down} strokeWidth="2"/></svg>바</button>
                </div></fieldset>}
